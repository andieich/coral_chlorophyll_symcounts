...
3.  Transfer tissue slurry into a pre-weighted (taraed) 500 mL beaker. Note the weight to estimate the volume of the tissue slurry (`w_slurry1` in metadata sheet). This will be used to estimate the total volume of the tissue slurry.
...
5.  Immediately transfer 40 g tissue slurry into pre-weigh (taraed) 50 mL tube (note as `w_slurry2` in metadata sheet). Try to be exact to keep the centrifuge used later balanced. Store tubes with tissue slurry in fridge when preparing the remaining samples.
